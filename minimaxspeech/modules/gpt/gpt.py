...
from minimaxspeech.modules.gpt.gpt_inference import GPT2InferenceModel
from minimaxspeech.modules.gpt.latent_encoder import ConditioningEncoder
from minimaxspeech.modules.gpt.perceiver_encoder import PerceiverResampler
...
if __name__ == "__main__":
    ...
    batch_size = 8
    text_inputs = torch.randint(0, 6680, (batch_size, 86)).to(device)
    text_lengths = torch.randint(10, 86, (batch_size,)).to(device)
    audio_codes = torch.randint(0, 1025, (batch_size, 201)).to(device)
    wav_lengths = torch.randint(10, 201, (batch_size,)).to(device)
    cond_mels = torch.randn(batch_size, 80, 516).to(device)
    cond_idxs = torch.zeros(batch_size, 2, dtype=torch.long).to(device)
    cond_idxs[:, 1] = 41114
    cond_lens = None

    from torchinfo import summary
    summary(model, input_data=(text_inputs, text_lengths, audio_codes, wav_lengths, cond_mels, cond_idxs))
