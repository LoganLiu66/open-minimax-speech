...
from minimaxspeech.modules.common.transformer.convolution import (
    CausalConv1d,
    CausalConv1dDownSample,
    CausalConv1dUpsample
)
from minimaxspeech.modules.common.transformer.activation import Snake
from minimaxspeech.modules.hifigan.f0_predictor import ConvRNNF0Predictor
from minimaxspeech.modules.hifigan.utils import get_padding, init_weights
...
class HiFTGenerator(nn.Module):
    ...
    def __init__(
        ...
        self.f0_predictor = ConvRNNF0Predictor(**f0_predictor)
...
def test_reconstruction():
    ...
    model.load_state_dict(hift_state_dict, strict=True)
    
    waveform, sr = torchaudio.load('data/LJ001-0001.wav')
    mel = mel_spectrogram(waveform)
    pred_gt, _ = model.inference(mel.to(device))
    torchaudio.save("data/LJ001-0001-hifigan.wav", pred_gt.cpu(), 22050)
...
def test_mel():
    ...
    im = ax.imshow(mel[0], aspect="auto", origin="lower", interpolation="none")
    plt.colorbar(im, ax=ax)
    plt.savefig('data/LJ001-0001-matcha.png')


if __name__ == '__main__':
    from omegaconf import OmegaConf
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    config = OmegaConf.load("./configs/hifigan_config.yaml")
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    model = HiFTGenerator(**config.hift).to(device)
    ...
    summary(model, input_data=(mel), depth=5)
