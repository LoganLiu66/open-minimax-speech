...
class MiniMaxSpeechModel:
    ...
    def load_checkpoint(self):
        ...
        logger.info(f"VQVAE model loaded")
    ...
    def generate(self, prompt_audio, text_tokens, only_spk_emb=False):
        """
        Args:
            prompt_audio: (1, t)
            text: (t_text)
            only_spk_emb: if True, only the speaker embedding is used for conditioning
        ...
        with torch.no_grad():
            ...
            logger.info(f"audio_tokens.shape: {audio_tokens.shape}, audio_tokens: {audio_tokens}")
            ...
            audio_tokens_len = torch.tensor([audio_tokens.shape[1]]).to(self.device)

            if only_spk_emb:
                latent = self.flow_matching.inference_without_prompt(
                    token=audio_tokens,
                    token_len=audio_tokens_len,
                    embedding=embedding
                )
            else:
                prompt_duration = 1
                prompt_audio_seg = prompt_audio[:, :, :int(22050 * prompt_duration)]
                prompt_feat = self.torch_mel_spectrogram_vq_vae(prompt_audio_seg)

                prompt_token = self.vq_vae.get_codebook_indices(prompt_feat) # (1, t', 80)
                prompt_token_len = torch.tensor([prompt_token.shape[1]]).to(self.device)

                prompt_z = self.flow_vae.encode(prompt_audio_seg).transpose(1, 2)
                prompt_z_len = torch.tensor([prompt_z.shape[1]]).to(self.device)

                latent = self.flow_matching.inference(
                    token=audio_tokens,
                    token_len=audio_tokens_len,
            ...
            """ FlowVAE inference """
            pred_gt = self.flow_vae.decode(latent)[0]
            return pred_gt